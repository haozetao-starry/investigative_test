...
module dds_sweep_tb;
    ...
    localparam integer MAX_SIM_CYCLES   = 5000;           // timeout
    ...
    localparam integer STEP_PERIOD_CLKS = 20;
    ...
    initial begin
        ...
        $fdisplay(csv_fd, "time_ns,cycle,f_word_hex,freq_hz,wave_out,sweep_busy,step_sync,phase_clr");

        $display("\n");
        $display("╔════════════════════════════════════════════════════════╗");
        $display("║         DDS  Sweep  Testbench                         ║");
        $display("╠════════════════════════════════════════════════════════╣");
        ...
        $display("║  Dwell        : %0d clocks                           ║", STEP_PERIOD_CLKS);
        $display("║  Repeat       : NO                                    ║");
    ...
    always @(posedge clk) begin
        if (rst_n) begin
            ...
            $fdisplay(csv_fd, "%0d,%0d,0x%08h,%0.0f,%0d,%b,%b,%b",
                      $time, cycle_count, f_word, freq_hz_approx,
                      wave_out, sweep_busy, step_sync, phase_clr);

            // ── console: step boundary ───────────────────────
            if (step_sync) begin
                step_count <= step_count + 1;
                $display("[%8t ns] ───────────────────────────────────────────", $time);
                $display("[%8t ns]  ▶ STEP %0d  freq ≈ %.0f Hz  f_word = 0x%08h",
                         $time, step_count + 1, freq_hz_approx, f_word);
            end
        end
    end
